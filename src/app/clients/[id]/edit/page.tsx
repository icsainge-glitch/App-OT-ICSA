...
import { getClientById, updateClient } from "@/actions/db-actions";
...
export default function EditClient({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: client, isLoading: isClientLoading } = useActionData<any>(() => getClientById(id), [id]);
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <Label htmlFor="rutCliente" className="font-bold flex items-center gap-2">
                    <Hash className="h-4 w-4 text-primary" /> RUT del Cliente <span className="opacity-60 lowercase font-medium ml-1 tracking-normal">(ej: 12345678-9)</span>
                  </Label>
                  <Input
                    id="rutCliente"
                    placeholder="12345678-9"
                    value={formData.rutCliente}
                    onChange={e => setFormData({ ...formData, rutCliente: formatRut(e.target.value) })}
                    className="h-12 border-none bg-muted/20 font-bold"
                    required
                  />
                </div>
                <div className="space-y-2">
