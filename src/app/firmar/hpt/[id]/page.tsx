...
function HPTRemoteSignaturePageContent({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    if (!isHptLoading) {
      if (!hpt) {
        setErrorMsg("El registro HPT no existe.");
        setIsValidating(false);
      } else if (hpt.status === 'Completado' && hpt.prevencionSignatureUrl) {
        setIsSubmitted(true);
        setIsValidating(false);
      } else if (!token || hpt.signatureToken !== token) {
        setErrorMsg("Token de firma no válido o acceso no autorizado.");
        setIsValidating(false);
      } else if (hpt.tokenExpiry && new Date(hpt.tokenExpiry) < new Date()) {
        setErrorMsg("El enlace de firma ha expirado (validez de 7 días).");
        setIsValidating(false);
      } else {
        setIsValidating(false);
        if (hpt.prevencionName) setPrevencionName(hpt.prevencionName);
      }
    }
  }, [hpt, isHptLoading, token]);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const result = await submitHPTRemoteSignature({
        id: id,
        token: token!,
        prevencionName,
      ...
      if (result.success) {
        toast({ title: "Firma Exitosa", description: "La HPT ha sido validada correctamente." });
        setIsSubmitted(true);
      } else {
  ...
  return (
    <div className="min-h-screen bg-muted/20 pb-12">
      <header className="bg-primary text-white p-6 sticky top-0 z-50 shadow-lg text-center">
        ...
        <p className="text-xs font-bold opacity-70 uppercase tracking-widest mt-1">Revisión de HPT</p>
      </header>
      ...
      <main className="container mx-auto px-4 mt-6 max-w-2xl space-y-6">
          ...
          <CardContent className="p-6 space-y-8">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-1">
                <Label className="uppercase text-[10px] font-black text-muted-foreground">Proyecto</Label>
                <div className="flex items-center gap-2 font-bold text-primary">
                  <Briefcase className="h-4 w-4" /> {hpt?.projectName || 'General'}
                </div>
              </div>
              <div className="space-y-1">
                <Label className="uppercase text-[10px] font-black text-muted-foreground">Supervisor / Fecha</Label>
                <div className="flex items-center gap-2 font-bold text-primary">
                  <User className="h-4 w-4" /> {hpt?.supervisorName}
                </div>
                <div className="flex items-center gap-2 text-xs font-bold text-muted-foreground mt-1">
                  <Clock className="h-3 w-3" /> {hpt?.fecha}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2">
              <Label className="uppercase text-[10px] font-black text-muted-foreground flex items-center gap-2">
                <MessageSquare className="h-3 w-3" /> Trabajo a Realizar
              </Label>
        ...
        <Card className="shadow-2xl border-none bg-white rounded-[2rem] overflow-hidden">
          <CardHeader className="bg-muted/10 p-6 border-b">
            ...
            <CardDescription className="text-xs font-bold">Confirme la revisión de la HPT mediante su firma digital.</CardDescription>
          </CardHeader>
          <CardContent className="p-6 space-y-6">
            <div className="space-y-4">
              ...
              <p className="text-[10px] text-center text-muted-foreground italic leading-tight px-4">
                Al firmar este documento, valido que los riesgos y medidas de seguridad han sido evaluados 
                correctamente para el inicio de las actividades.
              </p>
            </div>
...
export default function HPTRemoteSignaturePage({ params }: { params: Promise<{ id: string }> }) {
  return (
    <Suspense fallback={<div className="min-h-screen flex items-center justify-center font-black animate-pulse text-primary uppercase tracking-tighter">Cargando validación...</div>}>
      <HPTRemoteSignaturePageContent params={params} />
    </Suspense>
  );
}
