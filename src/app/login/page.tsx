...
import { loginAction, checkSysHealth } from "@/actions/auth-actions";
...
export default function LoginPage() {
  ...
  useEffect(() => {
    checkSysHealth().then(setHealth).catch(console.error);
  }, []);
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success && response.user) {
        toast({ title: "Bienvenido", description: `Acceso concedido as ${response.user.name}.` });
        window.location.href = "/dashboard";
      } else {
        toast({ variant: "destructive", title: "Error de Acceso", description: response.error || "Credenciales inválidas." });
        setLoading(false);
      }
    } catch (error: any) {
      console.error("[AUTH_ACTION_ERROR] Login failure:", error);
      toast({ variant: "destructive", title: "Error de Acceso", description: "Error de conexión: " + (error.message || "Desconocido") });
      setLoading(false);
    }
  };
        ...
        <CardContent className="pb-8 pt-4">
          <form onSubmit={handleAuth} className="space-y-5">
            ...
            <Button type="submit" className="w-full bg-primary h-14 text-lg font-black shadow-lg transition-all active:scale-95 rounded-xl" disabled={loading}>
              ...
              <Label htmlFor="password">{loading ? "Verificando..." : "Entrar al Portal"}</Label>
            </Button>
          </form>
