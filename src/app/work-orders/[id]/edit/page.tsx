...
import { getPersonnel, getClients, getActiveProjects, getOrderById, saveWorkOrderAndStatus } from "@/actions/db-actions";
...
export default function EditWorkOrder({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: allProjects } = useActionData<any[]>(() => getActiveProjects(user?.uid || '', isAdmin), [user?.uid, isAdmin]);
  ...
  const [formData, setFormData] = useState({
    ...
    detalleTecnico: [
      ...
      { elemento: "Puntos de red certificados", cantidad: "", observacion: "" },
      { elemento: "Cable UTP utilizado", cantidad: "", observacion: "" },
  ...
  useEffect(() => {
    if (order && !isInitialized) {
      ...
      setFormData({
        ...
        branch: order.branch || "Casa Matriz",
        address: order.address || "",
        interventionAddress: order.interventionAddress || "",
        building: order.building || "",
        floor: order.floor || "",
        tipoTrabajo: order.tipoTrabajo || "",
        tipoTrabajoOtro: order.tipoTrabajoOtro || "",
        detalleTecnico: Array.isArray(order.detalleTecnico) && order.detalleTecnico.length > 0 ? order.detalleTecnico : [
          { elemento: "Puntos de red instalados", cantidad: "", observacion: "" },
        ...
        puntosRed: Array.isArray(order.puntosRed) ? order.puntosRed : [],
        observacionesGenerales: order.observacionesGenerales || "",
        estadoTrabajo: order.estadoTrabajo || "Trabajo finalizado correctamente",
        description: order.description || "",
        techName: order.techName || userProfile?.nombre_t || "",
        techRut: order.techRut || userProfile?.rut_t || "",
        ...
        status: order.status || "",
        team: Array.isArray(order.team) ? order.team : [],
        teamIds: Array.isArray(order.teamIds) ? order.teamIds : [],
        condicionesCobro: Array.isArray(order.condicionesCobro) ? order.condicionesCobro : [],
        createdBy: order.createdBy || "",
        technicianId: order.technicianId || order.createdBy || "",
        photos: Array.isArray(order.photos) ? order.photos : (typeof order.photos === 'string' ? JSON.parse(order.photos) : []),
      });
  ...
  const onArchiveAndFinish = async () => {
    ...
    try {
      ...
      if (formData.clientReceiverEmail) {
        sendWorkOrderEmail({ order: updateData }).catch(err => console.error("Email error:", err));
      }
        ...
        <div className="container mx-auto px-4 h-16 flex items-center justify-between max-w-4xl">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              <h1 className="font-black text-sm md:text-lg uppercase tracking-tighter leading-tight">Editar Orden</h1>
              <span className="text-xs font-bold opacity-70 uppercase tracking-widest">OT {formatFolio(order?.folio)}</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button onClick={handleSendRemoteSignature} disabled={isSendingSignature || loading} variant="ghost" className="hidden md:flex h-10 px-4 font-black uppercase text-xs tracking-widest text-white hover:bg-white/10 rounded-xl transition-all">
      ...
      <div className="md:hidden fixed bottom-6 left-4 right-4 z-50 animate-slide-up" style={{ animationDelay: '300ms' }}>
        <div className="glassmorphism p-2 rounded-full shadow-[0_20px_50px_rgba(0,0,0,0.2)] flex gap-2 items-center">
          <Button
            ...
            disabled={loading}
            variant="ghost"
            className="flex-1 h-12 rounded-full font-black uppercase text-[9px] tracking-widest text-primary hover:bg-primary/10"
          >
          ...
          <Button
            ...
            disabled={loading}
            className="flex-[1.2] h-12 rounded-full font-black uppercase text-[9px] tracking-widest bg-emerald-500 text-white shadow-lg shadow-emerald-500/30"
          >
          ...
          <Button
            ...
            disabled={isSendingSignature || loading}
            className="h-12 w-12 rounded-full bg-primary/10 text-primary hover:bg-primary hover:text-white transition-colors"
          >
            <Send size={18} />
          </Button>
        </div>
        ...
        <form className="space-y-6">
          <Card className="shadow-xl border-none rounded-3xl overflow-hidden">
            ...
            <CardContent className="p-6">
              <div className="space-y-2">
                <Label className="font-black uppercase text-xs text-muted-foreground ml-1">Vincular a un proyecto activo</Label>
                <Select
          ...
          <Card className="shadow-xl border-none bg-white rounded-3xl overflow-hidden">
            ...
            <CardContent className="p-6 space-y-6">
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label className="font-black uppercase text-xs text-muted-foreground ml-1">Mandante / Empresa (Manual o Búsqueda)</Label>
                              ...
                              {clients?.map((client) => (
                                <CommandItem key={client.id} onSelect={() => handleSelectClient(client)} className="p-4 cursor-pointer rounded-2xl aria-selected:bg-primary aria-selected:text-white transition-all">
                                  ...
                                  <div className="flex flex-col">
                                    ...
                                    <span className="text-xs opacity-60 font-bold">{client.rutCliente}</span>
                                  </div>
                                </CommandItem>
                              ))}
                            </CommandGroup>
                ...
                <div className="space-y-2">
                  <Label className="font-black uppercase text-xs text-muted-foreground ml-1">RUT del Cliente / Empresa</Label>
                  <div className="relative">
                    <User className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-primary pointer-events-none" />
                    <Input placeholder="Ej: 12345678-9" value={formData.clientRut || ""} onChange={e => setFormData(prev => ({ ...prev, clientRut: e.target.value }))} className="h-14 pl-12 bg-muted/30 border-none rounded-2xl font-bold shadow-inner focus-visible:ring-primary" />
                  </div>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label className="font-black uppercase text-xs text-muted-foreground ml-1">Teléfono de Contacto</Label>
                    <div className="relative">
                  ...
                  <div className="space-y-2">
                    <Label className="font-black uppercase text-xs text-muted-foreground ml-1">Correo Electrónico</Label>
                    <div className="relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label className="font-black uppercase text-xs text-muted-foreground ml-1">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input placeholder="Ej: Av. Principal 123, Central" value={formData.address} onChange={e => setFormData(prev => ({ ...prev, address: e.target.value }))} className="h-14 pl-12 bg-muted/30 border-none rounded-2xl font-bold focus-visible:ring-primary shadow-inner" />
                    </div>
                  </div>
                </div>

                <div className="space-y-2">
                  <Label className="font-black uppercase text-xs text-muted-foreground ml-1">Dirección de la Sucursal</Label>
                  <div className="relative">
                    <MapPin className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-primary pointer-events-none" />
          ...
          <Card className="shadow-xl border-none bg-white rounded-3xl overflow-hidden">
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label className="font-black uppercase text-xs text-muted-foreground ml-1">Técnico Responsable ICSA</Label>
                  <div className="relative">
                ...
                <div className="space-y-2">
                  <Label className="font-black uppercase text-xs text-muted-foreground ml-1">RUT del Técnico <span className="opacity-60 lowercase font-medium ml-1 tracking-normal">(ej: 12345678-9)</span></Label>
                  <div className="relative">
                        ...
                        {(allPersonnel || []).map(person => (
                          <CommandItem key={person.id} onSelect={() => handleTeamSelect(person)} className="p-4 cursor-pointer rounded-2xl aria-selected:bg-primary aria-selected:text-white transition-all">
                            ...
                            <div className="flex flex-col">
                              ...
                              <span className="text-xs font-bold opacity-60">{person.rol_t}</span>
                            </div>
                          </CommandItem>
                        ))}
                      </CommandGroup>
          ...
          <Card className="shadow-xl border-none bg-white rounded-3xl overflow-hidden">
            ...
            <CardContent className="p-6 space-y-8">
              ...
              <div className="space-y-4">
                <Label className="font-black uppercase text-xs text-muted-foreground ml-1">1. Tipo de Trabajo</Label>
                <div className="flex flex-wrap gap-2">
                ...
                {formData.tipoTrabajo.includes('Otro') && (
              ...
              <div className="space-y-2">
                <Label className="font-black uppercase text-xs text-muted-foreground ml-1">2. Descripción del Trabajo <span className="text-[9px] lowercase opacity-60 font-medium">(Explicación simple para el cliente)</span></Label>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <Label className="font-black uppercase text-xs text-muted-foreground ml-1">3. Detalle Técnico del Trabajo</Label>
                  <Button type="button" onClick={() => setFormData(prev => ({ ...prev, detalleTecnico: [...prev.detalleTecnico, { elemento: "", cantidad: "", observacion: "" }] }))} variant="outline" size="sm" className="h-12 px-5 text-xs font-black uppercase rounded-2xl border-primary/20 text-primary hover:bg-primary/5 shadow-sm">
                ...
                {formData.detalleTecnico.length > 0 ? (
                  <div className="space-y-4">
                    ...
                    <div className="hidden md:block border border-primary/10 rounded-3xl overflow-hidden shadow-sm">
                      <table className="w-full text-sm text-left">
                        <thead className="bg-muted/50 text-xs uppercase font-black text-muted-foreground border-b border-primary/5">
                          <tr>
                  ...
                  <div className="bg-muted/10 border border-dashed rounded-3xl p-10 text-center text-sm font-bold text-muted-foreground flex flex-col items-center gap-3">
                    <PlusCircle className="h-10 w-10 opacity-20" />
                    No hay detalle técnico. Agregue una fila para comenzar.
                  </div>
                )}
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <Label className="font-black uppercase text-xs text-muted-foreground ml-1">4. Identificación de Puntos de Red (Si Aplica)</Label>
                  <Button type="button" onClick={() => setFormData(prev => ({ ...prev, puntosRed: [...prev.puntosRed, { id: "", ubicacion: "", puerto: "", resultado: "Certificado" }] }))} variant="outline" size="sm" className="h-12 px-5 text-xs font-black uppercase rounded-2xl border-primary/20 text-primary hover:bg-primary/5 shadow-sm">
                    <PlusCircle className="h-5 w-5 mr-2" /> Añadir Punto
                  </Button>
                </div>
                {formData.puntosRed.length > 0 ? (
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            ...
                            <div className="space-y-2">
                              ...
                              <Select value={punto.resultado} onValueChange={v => {
                                ...
                                <SelectTrigger className="h-12 bg-white border-none rounded-2xl text-xs font-black shadow-sm">
                                  <SelectValue />
                                </SelectTrigger>
                  ...
                  <div className="bg-muted/10 border border-dashed rounded-3xl p-10 text-center text-sm font-bold text-muted-foreground flex flex-col items-center gap-3">
                    <PlusCircle className="h-10 w-10 opacity-20" />
                    No hay puntos registrados. Ingrese un punto de red.
                  </div>
                )}
              </div>

              <div className="space-y-2">
                <Label className="font-black uppercase text-xs text-muted-foreground ml-1">5. Observaciones Finales</Label>
              ...
              <div className="space-y-4 pt-4 border-t border-dashed">
                <Label className="font-black uppercase text-xs text-muted-foreground ml-1">6. Estado del Trabajo</Label>
                <div className="flex flex-col sm:flex-row gap-3">
                  {[
                    ...
                    <Button
                      ...
                      className={`flex-1 h-16 rounded-2xl border-2 font-black uppercase text-xs tracking-widest shadow-sm transition-all ${formData.estadoTrabajo === estado.value ? estado.color : 'bg-transparent border-primary/10 hover:bg-muted/20 text-muted-foreground'}`}
                    >
                      {estado.label}
                    </Button>
                  ))}
              ...
              <div className="grid grid-cols-1 gap-8">
                <div className="space-y-4">
                  <SignaturePad
                    ...
                    initialValue={formData.techSignatureUrl || userProfile?.signatureUrl || undefined}
                    onSave={async (url) => {
                ...
                <div className="space-y-4 pt-4 border-t border-dashed">
                  <div className="space-y-2">
                    <Label className="text-xs font-black uppercase text-muted-foreground ml-1">Email del Cliente (Para Firma Remota)</Label>
                    <div className="relative">
                ...
                <div className="w-full flex flex-col items-center gap-4 bg-muted/5 p-6 rounded-2xl border border-dashed">
                        ...
                        <div className="space-y-8 pb-8">
                          ...
                          <div className="space-y-6">
                            ...
                            <div className="flex flex-wrap items-center gap-2">
                              <Badge className="bg-primary hover:bg-primary text-white border-none font-bold text-[9px] uppercase tracking-widest px-3">
                                Fecha: {order?.createdAt ? new Date(order.createdAt).toLocaleDateString('es-CL') : new Date().toLocaleDateString('es-CL')}
                              </Badge>
                              {formData.techName && (
                            ...
                            <div className="grid grid-cols-2 gap-4">
                              <div className="p-3 bg-muted/20 rounded-xl border border-primary/5 shadow-sm">
                                ...
                                <Badge className="bg-primary/10 text-primary hover:bg-primary/10 border-none font-bold uppercase text-xs">
                              ...
                              <div className="p-3 bg-muted/20 rounded-xl border border-primary/5 shadow-sm">
                                ...
                                <Badge className={`border-none font-bold uppercase text-xs ${formData.estadoTrabajo?.includes('Trabajo finalizado correctamente') ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-100' :
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-2">
                              <div className="flex flex-col col-span-2 md:col-span-1">
                                ...
                                {formData.clientRut && <p className="text-xs font-black text-muted-foreground ml-6">RUT: {formData.clientRut}</p>}
                                ...
                                {(formData.branch || formData.interventionAddress) && (
                                  <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                      ...
                                      {formData.interventionAddress && <p className="text-xs font-bold text-muted-foreground leading-tight mt-0.5">{formData.interventionAddress}</p>}
                                    </div>
                                  </div>
                                )}
                              </div>
                                      ...
                                      {formData.detalleTecnico.map((item, idx) => {
                                        ...
                                        return (
                                          <tr key={idx} className="bg-white">
                                            <td className="px-3 py-2 font-bold text-xs whitespace-normal leading-tight break-words max-w-[120px]">{item.elemento}</td>
                                            <td className="px-3 py-2 font-black text-center text-primary text-xs uppercase">{item.cantidad || "-"}</td>
                                            <td className="px-3 py-2 text-muted-foreground text-xs whitespace-normal leading-tight break-words">{item.observacion || "-"}</td>
                                          </tr>
                                        );
                                      })}
                                    </tbody>
                                  ...
                                  <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="divide-y divide-primary/5">
                                      {formData.puntosRed.map((punto, idx) => {
                                        ...
                                        return (
                                          <tr key={idx} className="bg-white">
                                            <td className="px-2 py-2 font-black text-xs text-center uppercase">{punto.id || "-"}</td>
                                            <td className="px-2 py-2 font-medium text-xs whitespace-normal leading-tight break-words">{punto.ubicacion || "-"}</td>
                                            <td className="px-2 py-2 font-medium text-xs uppercase whitespace-normal leading-tight break-words">{punto.puerto || "-"}</td>
                            ...
                            {formData.photos && formData.photos.length > 0 && (
                              <div className="space-y-2">
                                ...
                                <p className="text-xs text-muted-foreground font-bold pl-1 mb-2">{formData.photos.length} fotografía(s) adjuntas a la orden.</p>
                          ...
                          <div className="space-y-6">
                            <div className="text-center">
                              ...
                              <p className="text-xs text-muted-foreground font-medium">Complete sus datos y firme para confirmar la recepción.</p>
                            </div>

                            <div className="grid grid-cols-1 gap-4 pt-2">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                  <Label className="text-xs font-black uppercase text-muted-foreground ml-1">Nombre del Receptor <span className="text-destructive">*</span></Label>
                                  <Input
                                ...
                                <div className="space-y-2">
                                  <Label className="text-xs font-black uppercase text-muted-foreground ml-1">RUT del Receptor <span className="text-destructive">*</span></Label>
                                  <div className="relative">
                                    <User className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-primary pointer-events-none" />
                                    <Input
                              ...
                              <div className="space-y-2">
                                <Label className="text-xs font-black uppercase text-muted-foreground ml-1">Email del Receptor (Para envío PDF)</Label>
                                <Input
                                  type="email"
                                  placeholder="ejemplo@cliente.com"
                                  value={formData.clientReceiverEmail}
                  ...
                  {formData.clientSignatureUrl && formData.clientReceiverName && formData.clientReceiverRut && (
                    <div className="flex flex-col items-center mt-2 animate-fade-in">
                      <Badge className="bg-emerald-100 text-emerald-700 hover:bg-emerald-200 border-none px-4 py-1.5 gap-2 font-black uppercase text-xs">
                        <CheckCircle2 className="h-4 w-4" /> Firma Capturada Correctamente
                      </Badge>
                      <p className="text-xs font-bold text-muted-foreground mt-2">{formData.clientReceiverName} - {formData.clientReceiverRut}</p>
                    </div>
                  )}
                </div>
              </div>
